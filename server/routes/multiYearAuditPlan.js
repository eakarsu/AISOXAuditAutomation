...
// Multi-year audit plan optimization with risk-based resource allocation
// Feature: multi-year-audit-plan
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AISOXAuditAutomation" platform. Always return strict JSON only — no markdown, no commentary.',
    ...
    await persist(uid, 'multi-year-audit-plan', payload, result);
    res.json({ success: true, feature: 'multi-year-audit-plan', ...result });
  } catch (err) {
    console.error('multi-year-audit-plan analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AISOXAuditAutomation". Return strict JSON only.',
      `Feature: Multi-year audit plan optimization with risk-based resource allocation. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'multi-year-audit-plan', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'multi-year-audit-plan', title: 'Multi-year audit plan optimization with risk-based resource allocation', project: 'AISOXAuditAutomation' });
});
