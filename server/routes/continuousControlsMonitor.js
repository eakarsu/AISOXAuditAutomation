...
// Continuous controls monitoring with real-time exception alerts
// Feature: continuous-controls-monitor
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'continuous-controls-monitor', payload, result);
    res.json({ success: true, feature: 'continuous-controls-monitor', ...result });
  } catch (err) {
    console.error('continuous-controls-monitor analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Continuous controls monitoring with real-time exception alerts. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'continuous-controls-monitor', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'continuous-controls-monitor']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'continuous-controls-monitor', title: 'Continuous controls monitoring with real-time exception alerts', project: 'AISOXAuditAutomation' });
});
