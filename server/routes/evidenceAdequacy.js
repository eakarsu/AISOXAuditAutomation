...
// Evidence adequacy checker via RAG over PCAOB/COSO guidance
// Feature: evidence-adequacy
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'evidence-adequacy', payload, result);
    res.json({ success: true, feature: 'evidence-adequacy', ...result });
  } catch (err) {
    console.error('evidence-adequacy analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Evidence adequacy checker via RAG over PCAOB/COSO guidance. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'evidence-adequacy', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'evidence-adequacy', title: 'Evidence adequacy checker via RAG over PCAOB/COSO guidance', project: 'AISOXAuditAutomation' });
});
