...
// Feature: Multi-year audit plan optimization with risk-based resource allocation
...
const PRESETS = [
  { label: "Optimize 3-year plan", values: { prompt: "Optimize a 3-year SOX audit plan allocating limited auditor hours to the highest-risk processes first." } },
  { label: "Resource allocation", values: { prompt: "Recommend how to allocate a team of 4 auditors across 12 key processes based on risk scores." } },
]

export default function CfMultiYearAuditPlanOptimizationWithRisk() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-year-audit-plan-optimization-with-risk-based-resource-allocation/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-multi-year-audit-plan-optimization-with-risk-based-resource-allocation', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-multi-year-audit-plan-optimization-with-risk-based-resource-allocation/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Multi-year audit plan optimization with risk-based resource allocation</h1>
