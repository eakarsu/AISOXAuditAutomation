...
// Feature: Regulatory change digest ingesting SEC/PCAOB updates, flagging control impacts
...
const PRESETS = [
  { label: "Latest SEC/PCAOB", values: { prompt: "Summarize recent SEC and PCAOB updates relevant to SOX 404 and flag which of our controls may be impacted." } },
  { label: "Impact assessment", values: { prompt: "Given a new PCAOB inspection focus on revenue controls, identify the controls and evidence we should strengthen." } },
]

export default function CfRegulatoryChangeDigestIngestingSecPcaobUpdates() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-change-digest-ingesting-sec-pcaob-updates-flagging/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-regulatory-change-digest-ingesting-sec-pcaob-updates-flagging', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-regulatory-change-digest-ingesting-sec-pcaob-updates-flagging/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Regulatory change digest ingesting SEC/PCAOB updates, flagging control impacts</h1>
