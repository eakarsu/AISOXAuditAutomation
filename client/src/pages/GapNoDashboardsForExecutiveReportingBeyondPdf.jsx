...
// Feature: No dashboards for executive reporting beyond PDF export
...
const PRESETS = [
  { label: "Audit committee KPIs", values: { prompt: "Recommend the key metrics and visualizations an executive SOX dashboard should show the audit committee for the current quarter." } },
  { label: "Control health view", values: { prompt: "Design a control-health dashboard summarizing tested vs. untested controls, deficiencies by severity, and remediation status." } },
]

export default function GapNoDashboardsForExecutiveReportingBeyondPdf() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-dashboards-for-executive-reporting-beyond-pdf-export/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-dashboards-for-executive-reporting-beyond-pdf-export', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-dashboards-for-executive-reporting-beyond-pdf-export/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No dashboards for executive reporting beyond PDF export</h1>
