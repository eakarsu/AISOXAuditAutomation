...
export default function CfAnomalyDetectionInGlPayrollApTransaction() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-anomaly-detection-in-gl-payroll-ap-transaction-feeds/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-anomaly-detection-in-gl-payroll-ap-transaction-feeds', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-anomaly-detection-in-gl-payroll-ap-transaction-feeds/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Anomaly detection in GL/payroll/AP transaction feeds for control gap discovery</h1>
