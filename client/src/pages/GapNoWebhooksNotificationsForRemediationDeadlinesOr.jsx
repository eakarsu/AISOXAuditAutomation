...
// Feature: No webhooks/notifications for remediation deadlines or new findings
...
const PRESETS = [
  { label: "Deadline reminders", values: { prompt: "Design webhook notifications for remediation deadlines at 7, 3, and 1 days before due, including payload fields and channels." } },
  { label: "New-finding alert", values: { prompt: "Specify a notification flow that alerts control owners and the audit lead when a new high-severity finding is logged." } },
]

export default function GapNoWebhooksNotificationsForRemediationDeadlinesOr() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-webhooks-notifications-for-remediation-deadlines-or-new/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-webhooks-notifications-for-remediation-deadlines-or-new', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-webhooks-notifications-for-remediation-deadlines-or-new/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No webhooks/notifications for remediation deadlines or new findings</h1>
