...
export default function CfEvidenceAdequacyCheckerViaRagOverPcaob() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-evidence-adequacy-checker-via-rag-over-pcaob-coso/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-evidence-adequacy-checker-via-rag-over-pcaob-coso', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-evidence-adequacy-checker-via-rag-over-pcaob-coso/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Evidence adequacy checker via RAG over PCAOB/COSO guidance</h1>
