...
const PRESETS = [
  { label: "Finding approval flow", values: { prompt: "Design a multi-step approval/sign-off workflow for audit findings: preparer, reviewer, and audit-lead sign-off with state transitions." } },
  { label: "Segregation of duties", values: { prompt: "Recommend approval routing that enforces segregation of duties for remediation sign-offs." } },
]

export default function GapNoWorkflowApprovalsSignOffsForFindings() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-workflow-approvals-sign-offs-for-findings-no-approval/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-workflow-approvals-sign-offs-for-findings-no-approval', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-workflow-approvals-sign-offs-for-findings-no-approval/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No workflow approvals/sign-offs for findings (no approval routes)</h1>
