...
const PRESETS = [
  { label: "Audit trail design", values: { prompt: "Propose an audit-trail subsystem capturing who/what/when for every change to controls, evidence, and findings, with retention guidance." } },
  { label: "Tamper evidence", values: { prompt: "Recommend tamper-evident logging (hashing, append-only) suitable for SOX evidence integrity." } },
]

export default function GapNoDedicatedAuditTrailSubsystemDespiteDomain() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-dedicated-audit-trail-subsystem-despite-domain-requirement/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-dedicated-audit-trail-subsystem-despite-domain-requirement', project: 'AISOXAuditAutomation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-dedicated-audit-trail-subsystem-despite-domain-requirement/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No dedicated audit trail subsystem (despite domain requirement)</h1>
